...
export async function generateHyDEAnswer(query: string): Promise<string> {
  ...
  try {
    logger.debug(`[ArcRift] HyDE: "${query.slice(0, 40)}..."`);
    const hypotheticalAnswer = await llm(prompt, 100);
    return hypotheticalAnswer.trim();
  } catch (err) {
    logger.warn(`[ArcRift] HyDE generation failed, falling back to raw query: ${err instanceof Error ? err.message : String(err)}`);
    return query;
  }
}
